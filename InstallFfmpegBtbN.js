...
function Script()
{
    ...
    try {
        ...
        for (let asset of releaseData.assets) {
            if (asset.browser_download_url.endsWith('ffmpeg-master-latest-linux64-gpl.tar.xz')) {
        ...
        if (assetUrl) {
            ...
            Logger.ELog('No asset found ending with ffmpeg-master-latest-linux64-gpl.tar.xz');
            return -1;
        }
    } catch (error) {
        Logger.ELog('Error parsing JSON: ' + error);
